...
function state(partial: Record<string, unknown>): AppState {
  return { chills: [], thermostat: null, rooms: [], metrics: [], ...partial } as unknown as AppState;
}
...
describe("defaultCardIds", () => {
  it("orders lights, thermostat, chills, then metric rooms with a visible metric", () => {
    ...
    expect(defaultCardIds(s)).toEqual(["lights", "thermostat", "climate.zolder", "climate.speelkamer", "woonkamer"]);
  });

  it("returns nothing when there are no rooms, thermostat, chills, or visible metrics", () => {
    expect(defaultCardIds(state({}))).toEqual([]);
  });
});
